# -*- coding: gbk -*-
###############################################################################
#   农信银系统：来账.中心类操作(1.本地操作 2.中心回执).卡通兑来帐
#==============================================================================
#   交易文件:   TRCC006_1139.py
#   公司名称：  北京赞同科技有限公司
#   作    者：  liyj 
#   修改时间:   2008-11-05
###############################################################################
...
import rccpsMap1139CTradeContext2Dwtrbka_dict,rccpsDBTrcc_atcbka


#=====================交易前处理(登记流水,中心前处理)==========================
def SubModuleDoFst():
    AfaLoggerFunc.tradeInfo( '***农信银系统：来账.中心类操作(1.本地操作).卡通兑来帐请求报文接收[TRCC006_1139]进入***' )
    ...
    if TradeContext.PRCCO == 'RCCI0000':
        wtrbka_dict = {}
        if not rccpsMap1139CTradeContext2Dwtrbka_dict.map(wtrbka_dict):
    ...
    AfaLoggerFunc.tradeInfo( '***农信银系统：来账.中心类操作(1.本地操作).卡通兑来帐请求报文接收[TRCC006_1139]退出***' )
    return True


#=====================交易后处理===============================================
def SubModuleDoSnd():
    AfaLoggerFunc.tradeInfo( '***农信银系统：来账.中心类操作(2.中心回执).卡通兑来帐请求报文接收[TRCC006_1139]进入***' )
    ...
    AfaLoggerFunc.tradeInfo( '***农信银系统：来账.中心类操作(2.中心回执).卡通兑来帐请求报文接收[TRCC006_1139]退出***' )
    return True
