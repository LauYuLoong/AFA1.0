# -*- coding: gbk -*-
###############################################################################
#   农信银系统：往账.中心类操作模板(1.本地操作 2.中心记帐).异转本
#==============================================================================
#   交易文件:   TRCC002_8564.py
#   公司名称：  北京赞同科技有限公司
#   作    者：  刘雨龙
#   修改时间:   2008-10-30
###############################################################################
...
import rccpsDBFunc,rccpsState,rccpsDBTrcc_pamtbl,rccpsMap8564CTradeContext2Dwtrbka_dict
import rccpsHostFunc,HostContext


#=====================交易前处理(登记流水,主机前处理)==========================
def SubModuleDoFst():
    AfaLoggerFunc.tradeInfo( '***农信银系统: 通存通兑.本地类操作交易[RCC003_8564]交易前处理进入***' )
    ...
    AfaLoggerFunc.tradeInfo( '>>>判断存折/卡信息结束' )
        
        
    #=====卡 处理，需要确认折如何处理====
    #=====收款人账号检查====
    TradeContext.ACCNO    = TradeContext.PYEACC     #收款人账户
    TradeContext.HostCode = '8810'

    rccpsHostFunc.CommHost( TradeContext.HostCode )
       
    if TradeContext.errorCode != '0000':
        return AfaFlowControl.ExitThisFlow('S999','查询收款人账户信息出错')
    else:
        if ((TradeContext.ACCCD == '0428' and TradeContext.ACCEM == '21111') is False):
            #=====个人卡=====
            return AfaFlowControl.ExitThisFlow('S999','账号类型错，必须为个人结算户')
            
        if( not (TradeContext.ACCST =='0' or TradeContext.ACCST =='2')):
            return AfaFlowControl.ExitThisFlow('S999','账号状态异常')
    
    AfaLoggerFunc.tradeInfo( '>>>判断收款人账户结束' )    
    ...
    AfaLoggerFunc.tradeInfo( '>>>根据交易金额判断证件类型/号码结束' )
    ...
    #=====字段赋值====
    TradeContext.OPRNO   =  PL_TDOPRNO_YZB          #业务种类
    TradeContext.DCFLG   =  PL_DCFLG_DEB           #借贷标识

    #=====字典赋值，插入数据库====
    wtrbka_dict = {}
    if not rccpsMap8564CTradeContext2Dwtrbka_dict.map(wtrbka_dict):
    ...
    AfaLoggerFunc.tradeInfo( '>>>字典赋值结束' )
    ...
    AfaLoggerFunc.tradeInfo( '>>>插入通存通兑业务登记簿结束' )
    ...
    AfaLoggerFunc.tradeInfo(">>>结束发送农信银中心处理")

    AfaLoggerFunc.tradeInfo( '***农信银系统: 通存通兑.本地类操作交易[RCC003_8564]交易前处理退出***' )
    return True
#=====================交易后处理===============================================
def SubModuleDoSnd():
    AfaLoggerFunc.tradeInfo( '***农信银系统: 通存通兑.中心类操作交易[RCC003_8564]交易后处理进入***' )
    ...
    AfaLoggerFunc.tradeInfo( '***农信银系统: 通存通兑.中心类操作交易[RCC003_8564]交易后处理退出***' )
