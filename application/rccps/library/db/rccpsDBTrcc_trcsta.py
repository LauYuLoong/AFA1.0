# -*- coding: gbk -*-
##################################################################
#   农信银系统 rcc_trcsta 数据库表操作函数
#
#   作    者：  关彬捷
#   程序文件:   rccpsDBTrcc_trcsta.py
#   修改时间:   Tue Dec 16 19:39:24 2008
##################################################################
import AfaDBFunc,AfaLoggerFunc
from types import *

table_tuple = ('NCCWKDAT', 'BESBNO', 'TRCCO', 'BRSFLG', 'BTOPSB', 'BEACSB', 'TCNT', 'TAMT', 'ISDEAL', 'STRINFO', 'NOTE1', 'NOTE2', 'NOTE3', 'NOTE4')
table_dict = {'BEACSB': 'S', 'NCCWKDAT': 'S', 'BRSFLG': 'S', 'TRCCO': 'S', 'TCNT': 'N', 'NOTE3': 'S', 'NOTE2': 'S', 'BTOPSB': 'S', 'STRINFO': 'S', 'NOTE4': 'S', 'NOTE1': 'S', 'BESBNO': 'S', 'ISDEAL': 'S', 'TAMT': 'F'}
    
def chk(where_dict):
    for item in where_dict.keys():
        if item not in table_tuple:
            AfaLoggerFunc.tradeError( '表[rcc_trcsta]中无列[' + item + ']' )
            return False
    return True
    
def count( wheresql ):

    sql = "SELECT count(*) FROM rcc_trcsta WHERE " + wheresql
...
def selectu( where_dict ):
    ...
    sql = "SELECT NCCWKDAT,BESBNO,TRCCO,BRSFLG,BTOPSB,BEACSB,TCNT,TAMT,ISDEAL,STRINFO,NOTE1,NOTE2,NOTE3,NOTE4 FROM rcc_trcsta WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
    records = AfaDBFunc.SelectSql( sql )
    if (records == None):
        return None
...
def insert( insert_dict ):
    ...
    sql = "INSERT INTO rcc_trcsta (" + ','.join(table_tuple) + ") VALUES ( "
...
def update( update_dict,where_dict ):
    ...
    sql = "UPDATE rcc_trcsta SET " + updatesql[0:-1] + " WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
...
def delete( where_dict ):
    ...
    sql = "DELETE FROM rcc_trcsta WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
