# -*- coding: gbk -*-
##################################################################
#   农信银系统 rcc_atcbka 数据库表操作函数
#
#   作    者：  关彬捷
#   程序文件:   rccpsDBTrcc_atcbka.py
#   修改时间:   Tue Dec 16 19:39:19 2008
##################################################################
import AfaDBFunc,AfaLoggerFunc
from types import *

table_tuple = ('BJEDTE', 'BSPSQN', 'BRSFLG', 'BESBNO', 'BEACSB', 'BETELR', 'BEAUUS', 'BEAUPS', 'TERMID', 'OPRNO', 'OPRATTNO', 'NCCWKDAT', 'TRCCO', 'TRCDAT', 'TRCNO', 'MSGFLGNO', 'ORTRCCO', 'ORTRCDAT', 'ORTRCNO', 'ORMFN', 'SNDMBRCO', 'RCVMBRCO', 'SNDBNKCO', 'SNDBNKNM', 'RCVBNKCO', 'RCVBNKNM', 'BOJEDT', 'BOSPSQ', 'RESNCO', 'PRCCO', 'STRINFO', 'NOTE1', 'NOTE2', 'NOTE3', 'NOTE4')
table_dict = {'BEAUUS': 'S', 'TRCDAT': 'S', 'ORTRCCO': 'S', 'TRCCO': 'S', 'PRCCO': 'S', 'SNDMBRCO': 'S', 'RCVMBRCO': 'S', 'BJEDTE': 'S', 'BSPSQN': 'S', 'RCVBNKCO': 'S', 'OPRATTNO': 'S', 'TERMID': 'S', 'BOSPSQ': 'S', 'ORMFN': 'S', 'ORTRCNO': 'S', 'SNDBNKNM': 'S', 'BEACSB': 'S', 'RCVBNKNM': 'S', 'NCCWKDAT': 'S', 'BRSFLG': 'S', 'ORTRCDAT': 'S', 'STRINFO': 'S', 'MSGFLGNO': 'S', 'BOJEDT': 'S', 'RESNCO': 'S', 'NOTE1': 'S', 'OPRNO': 'S', 'NOTE3': 'S', 'NOTE2': 'S', 'NOTE4': 'S', 'BEAUPS': 'S', 'SNDBNKCO': 'S', 'TRCNO': 'S', 'BESBNO': 'S', 'BETELR': 'S'}
    
def chk(where_dict):
    for item in where_dict.keys():
        if item not in table_tuple:
            AfaLoggerFunc.tradeError( '表[rcc_atcbka]中无列[' + item + ']' )
            return False
    return True
    
def count( wheresql ):

    sql = "SELECT count(*) FROM rcc_atcbka WHERE " + wheresql
...
def selectu( where_dict ):
    ...
    sql = "SELECT BJEDTE,BSPSQN,BRSFLG,BESBNO,BEACSB,BETELR,BEAUUS,BEAUPS,TERMID,OPRNO,OPRATTNO,NCCWKDAT,TRCCO,TRCDAT,TRCNO,MSGFLGNO,ORTRCCO,ORTRCDAT,ORTRCNO,ORMFN,SNDMBRCO,RCVMBRCO,SNDBNKCO,SNDBNKNM,RCVBNKCO,RCVBNKNM,BOJEDT,BOSPSQ,RESNCO,PRCCO,STRINFO,NOTE1,NOTE2,NOTE3,NOTE4 FROM rcc_atcbka WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
    records = AfaDBFunc.SelectSql( sql )
    if (records == None):
        return None
...
def selectm( start_no, sel_size, wheresql, ordersql):
    ...
    sql = "SELECT BJEDTE,BSPSQN,BRSFLG,BESBNO,BEACSB,BETELR,BEAUUS,BEAUPS,TERMID,OPRNO,OPRATTNO,NCCWKDAT,TRCCO,TRCDAT,TRCNO,MSGFLGNO,ORTRCCO,ORTRCDAT,ORTRCNO,ORMFN,SNDMBRCO,RCVMBRCO,SNDBNKCO,SNDBNKNM,RCVBNKCO,RCVBNKNM,BOJEDT,BOSPSQ,RESNCO,PRCCO,STRINFO,NOTE1,NOTE2,NOTE3,NOTE4 FROM (SELECT row_number() over(" + ordersql + ") as rowid,BJEDTE,BSPSQN,BRSFLG,BESBNO,BEACSB,BETELR,BEAUUS,BEAUPS,TERMID,OPRNO,OPRATTNO,NCCWKDAT,TRCCO,TRCDAT,TRCNO,MSGFLGNO,ORTRCCO,ORTRCDAT,ORTRCNO,ORMFN,SNDMBRCO,RCVMBRCO,SNDBNKCO,SNDBNKNM,RCVBNKCO,RCVBNKNM,BOJEDT,BOSPSQ,RESNCO,PRCCO,STRINFO,NOTE1,NOTE2,NOTE3,NOTE4 from rcc_atcbka WHERE " + wheresql + " ) as tab1 where tab1.rowid >= " + str(start_no)
    AfaLoggerFunc.tradeInfo(sql)
...
def insert( insert_dict ):
    ...
    sql = "INSERT INTO rcc_atcbka (" + ','.join(table_tuple) + ") VALUES ( "
...
def update( update_dict,where_dict ):
    ...
    sql = "UPDATE rcc_atcbka SET " + updatesql[0:-1] + " WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
...
def delete( where_dict ):
    ...
    sql = "DELETE FROM rcc_atcbka WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
