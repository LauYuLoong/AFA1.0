# -*- coding: gbk -*-
##################################################################
#   农信银系统 rcc_jstbka 数据库表操作函数
#
#   作    者：  关彬捷
#   程序文件:   rccpsDBTrcc_jstbka.py
#   修改时间:   Tue Dec 16 19:39:21 2008
##################################################################
import AfaDBFunc,AfaLoggerFunc
from types import *

table_tuple = ('BSPSQN', 'BJEDTE', 'BRSFLG', 'BESBNO', 'BETELR', 'BEAUUS', 'BEAUPS', 'TERMID', 'DCFLG', 'OPRNO', 'NCCWKDAT', 'TRCCO', 'TRCDAT', 'TRCNO', 'MSGFLGNO', 'SNDMBRCO', 'RCVMBRCO', 'SNDBNKCO', 'SNDBNKNM', 'RCVBNKCO', 'RCVBNKNM', 'CUR', 'OCCAMT', 'CHRGTYP', 'CUSCHRG', 'PYRACC', 'PYRNAM', 'PYEACC', 'PYENAM', 'STRINFO', 'CERTTYPE', 'CERTNO', 'CBFLG', 'NOTE1', 'NOTE2', 'NOTE3', 'NOTE4', 'NOTE5', 'NOTE6', 'BOJEDT', 'BOSPSQ', 'PRCCO')
table_dict = {'BEAUUS': 'S', 'TRCDAT': 'S', 'MSGFLGNO': 'S', 'PYRACC': 'S', 'STRINFO': 'S', 'SNDMBRCO': 'S', 'PYRNAM': 'S', 'RCVMBRCO': 'S', 'OCCAMT': 'F', 'PYENAM': 'S', 'NCCWKDAT': 'S', 'BJEDTE': 'S', 'DCFLG': 'S', 'BSPSQN': 'S', 'RCVBNKCO': 'S', 'CERTTYPE': 'S', 'TERMID': 'S', 'BOSPSQ': 'S', 'SNDBNKNM': 'S', 'TRCCO': 'S', 'RCVBNKNM': 'S', 'CUR': 'S', 'BRSFLG': 'S', 'CERTNO': 'S', 'PYEACC': 'S', 'CHRGTYP': 'S', 'PRCCO': 'S', 'BOJEDT': 'S', 'CUSCHRG': 'F', 'NOTE1': 'S', 'OPRNO': 'S', 'NOTE3': 'S', 'NOTE2': 'S', 'NOTE5': 'S', 'NOTE4': 'S', 'NOTE6': 'S', 'BEAUPS': 'S', 'SNDBNKCO': 'S', 'TRCNO': 'S', 'BESBNO': 'S', 'BETELR': 'S', 'CBFLG': 'S'}
    
def chk(where_dict):
    for item in where_dict.keys():
        if item not in table_tuple:
            AfaLoggerFunc.tradeError( '表[rcc_jstbka]中无列[' + item + ']' )
            return False
    return True
    
def count( wheresql ):

    sql = "SELECT count(*) FROM rcc_jstbka WHERE " + wheresql
...
def selectu( where_dict ):
    ...
    sql = "SELECT BSPSQN,BJEDTE,BRSFLG,BESBNO,BETELR,BEAUUS,BEAUPS,TERMID,DCFLG,OPRNO,NCCWKDAT,TRCCO,TRCDAT,TRCNO,MSGFLGNO,SNDMBRCO,RCVMBRCO,SNDBNKCO,SNDBNKNM,RCVBNKCO,RCVBNKNM,CUR,OCCAMT,CHRGTYP,CUSCHRG,PYRACC,PYRNAM,PYEACC,PYENAM,STRINFO,CERTTYPE,CERTNO,CBFLG,NOTE1,NOTE2,NOTE3,NOTE4,NOTE5,NOTE6,BOJEDT,BOSPSQ,PRCCO FROM rcc_jstbka WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
    records = AfaDBFunc.SelectSql( sql )
    if (records == None):
        return None
...
def selectm( start_no, sel_size, wheresql, ordersql):
    ...
    sql = "SELECT BSPSQN,BJEDTE,BRSFLG,BESBNO,BETELR,BEAUUS,BEAUPS,TERMID,DCFLG,OPRNO,NCCWKDAT,TRCCO,TRCDAT,TRCNO,MSGFLGNO,SNDMBRCO,RCVMBRCO,SNDBNKCO,SNDBNKNM,RCVBNKCO,RCVBNKNM,CUR,OCCAMT,CHRGTYP,CUSCHRG,PYRACC,PYRNAM,PYEACC,PYENAM,STRINFO,CERTTYPE,CERTNO,CBFLG,NOTE1,NOTE2,NOTE3,NOTE4,NOTE5,NOTE6,BOJEDT,BOSPSQ,PRCCO FROM (SELECT row_number() over(" + ordersql + ") as rowid,BSPSQN,BJEDTE,BRSFLG,BESBNO,BETELR,BEAUUS,BEAUPS,TERMID,DCFLG,OPRNO,NCCWKDAT,TRCCO,TRCDAT,TRCNO,MSGFLGNO,SNDMBRCO,RCVMBRCO,SNDBNKCO,SNDBNKNM,RCVBNKCO,RCVBNKNM,CUR,OCCAMT,CHRGTYP,CUSCHRG,PYRACC,PYRNAM,PYEACC,PYENAM,STRINFO,CERTTYPE,CERTNO,CBFLG,NOTE1,NOTE2,NOTE3,NOTE4,NOTE5,NOTE6,BOJEDT,BOSPSQ,PRCCO from rcc_jstbka WHERE " + wheresql + " ) as tab1 where tab1.rowid >= " + str(start_no)
    AfaLoggerFunc.tradeInfo(sql)
...
def insert( insert_dict ):
    ...
    sql = "INSERT INTO rcc_jstbka (" + ','.join(table_tuple) + ") VALUES ( "
...
def update( update_dict,where_dict ):
    ...
    sql = "UPDATE rcc_jstbka SET " + updatesql[0:-1] + " WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
...
def delete( where_dict ):
    ...
    sql = "DELETE FROM rcc_jstbka WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
