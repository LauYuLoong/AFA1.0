# -*- coding: gbk -*-
##################################################################
#   农信银系统 rcc_sstlog 数据库表操作函数
#
#   作    者：  关彬捷
#   程序文件:   rccpsDBTrcc_sstlog.py
#   修改时间:   Tue Dec 16 19:39:23 2008
##################################################################
import AfaDBFunc,AfaLoggerFunc
from types import *

table_tuple = ('BJEDTE', 'BSPSQN', 'BCURSQ', 'BESBNO', 'BEACSB', 'BETELR', 'BEAUUS', 'FEDT', 'RBSQ', 'TRDT', 'TLSQ', 'SBAC', 'ACNM', 'RBAC', 'OTNM', 'DASQ', 'MGID', 'PRCCO', 'STRINFO', 'BCSTAT', 'BDWFLG', 'PRTCNT', 'BJETIM', 'NOTE1', 'NOTE2', 'NOTE3', 'NOTE4')
table_dict = {'RBSQ': 'S', 'BCURSQ': 'N', 'BEAUUS': 'S', 'PRCCO': 'S', 'RBAC': 'S', 'SBAC': 'S', 'BJEDTE': 'S', 'OTNM': 'S', 'BSPSQN': 'S', 'TRDT': 'S', 'MGID': 'S', 'PRTCNT': 'N', 'BEACSB': 'S', 'DASQ': 'S', 'BJETIM': 'S', 'BCSTAT': 'S', 'FEDT': 'S', 'STRINFO': 'S', 'TLSQ': 'S', 'ACNM': 'S', 'NOTE1': 'S', 'NOTE3': 'S', 'NOTE2': 'S', 'NOTE4': 'S', 'BESBNO': 'S', 'BDWFLG': 'S', 'BETELR': 'S'}
    
def chk(where_dict):
    for item in where_dict.keys():
        if item not in table_tuple:
            AfaLoggerFunc.tradeError( '表[rcc_sstlog]中无列[' + item + ']' )
            return False
    return True
    
def count( wheresql ):

    sql = "SELECT count(*) FROM rcc_sstlog WHERE " + wheresql
...
def insert( insert_dict ):
    ...
    sql = "INSERT INTO rcc_sstlog (" + ','.join(table_tuple) + ") VALUES ( "
...
def update( update_dict,where_dict ):
    ...
    sql = "UPDATE rcc_sstlog SET " + updatesql[0:-1] + " WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
...
def delete( where_dict ):
    ...
    sql = "DELETE FROM rcc_sstlog WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
