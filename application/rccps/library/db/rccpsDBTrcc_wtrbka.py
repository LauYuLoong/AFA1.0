# -*- coding: gbk -*-
##################################################################
#   农信银系统 rcc_wtrbka 数据库表操作函数
#
#   作    者：  关彬捷
#   程序文件:   rccpsDBTrcc_wtrbka.py
#   修改时间:   Tue Dec 16 19:39:24 2008
##################################################################
import AfaDBFunc,AfaLoggerFunc
from types import *

table_tuple = ('BJEDTE', 'BSPSQN', 'BRSFLG', 'BESBNO', 'BEACSB', 'BETELR', 'BEAUUS', 'BEAUPS', 'TERMID', 'BBSSRC', 'DASQ', 'DCFLG', 'OPRNO', 'OPRATTNO', 'NCCWKDAT', 'TRCCO', 'TRCDAT', 'TRCNO', 'MSGFLGNO', 'COTRCDAT', 'COTRCNO', 'COMSGFLGNO', 'SNDMBRCO', 'RCVMBRCO', 'SNDBNKCO', 'SNDBNKNM', 'RCVBNKCO', 'RCVBNKNM', 'CUR', 'OCCAMT', 'CHRGTYP', 'LOCCUSCHRG', 'CUSCHRG', 'PYRTYP', 'PYRACC', 'PYRNAM', 'PYRADDR', 'PYETYP', 'PYEACC', 'PYENAM', 'PYEADDR', 'STRINFO', 'CERTTYPE', 'CERTNO', 'BNKBKNO', 'BNKBKBAL', 'NOTE1', 'NOTE2', 'NOTE3', 'NOTE4')
table_dict = {'BEAUUS': 'S', 'TRCDAT': 'S', 'MSGFLGNO': 'S', 'TRCCO': 'S', 'STRINFO': 'S', 'SNDMBRCO': 'S', 'PYRNAM': 'S', 'CERTTYPE': 'S', 'RCVMBRCO': 'S', 'OCCAMT': 'F', 'PYENAM': 'S', 'COTRCNO': 'S', 'CUR': 'S', 'BJEDTE': 'S', 'DCFLG': 'S', 'BNKBKNO': 'S', 'BSPSQN': 'S', 'RCVBNKCO': 'S', 'OPRATTNO': 'S', 'BBSSRC': 'S', 'COTRCDAT': 'S', 'TERMID': 'S', 'SNDBNKNM': 'S', 'BEACSB': 'S', 'RCVBNKNM': 'S', 'PYEADDR': 'S', 'NCCWKDAT': 'S', 'BRSFLG': 'S', 'DASQ': 'S', 'BNKBKBAL': 'F', 'CERTNO': 'S', 'PYEACC': 'S', 'CHRGTYP': 'S', 'COMSGFLGNO': 'S', 'PYRACC': 'S', 'PYRADDR': 'S', 'CUSCHRG': 'F', 'LOCCUSCHRG': 'F', 'NOTE1': 'S', 'OPRNO': 'S', 'PYRTYP': 'S', 'NOTE2': 'S', 'NOTE4': 'S', 'PYETYP': 'S', 'BEAUPS': 'S', 'SNDBNKCO': 'S', 'TRCNO': 'S', 'BESBNO': 'S', 'BETELR': 'S', 'NOTE3': 'S'}
    
def chk(where_dict):
    for item in where_dict.keys():
        if item not in table_tuple:
            AfaLoggerFunc.tradeError( '表[rcc_wtrbka]中无列[' + item + ']' )
            return False
    return True
    
def count( wheresql ):

    sql = "SELECT count(*) FROM rcc_wtrbka WHERE " + wheresql
...
def insert( insert_dict ):
    ...
    sql = "INSERT INTO rcc_wtrbka (" + ','.join(table_tuple) + ") VALUES ( "
...
def update( update_dict,where_dict ):
    ...
    sql = "UPDATE rcc_wtrbka SET " + updatesql[0:-1] + " WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
...
def delete( where_dict ):
    ...
    sql = "DELETE FROM rcc_wtrbka WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
