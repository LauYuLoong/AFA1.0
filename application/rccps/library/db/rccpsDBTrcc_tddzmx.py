# -*- coding: gbk -*-
##################################################################
#   农信银系统 rcc_tddzmx 数据库表操作函数
#
#   作    者：  关彬捷
#   程序文件:   rccpsDBTrcc_tddzmx.py
#   修改时间:   Tue Dec 16 19:39:23 2008
##################################################################
import AfaDBFunc,AfaLoggerFunc
from types import *

table_tuple = ('SNDBNKCO', 'TRCDAT', 'TRCNO', 'NCCWKDAT', 'TRCCO', 'MSGTYPCO', 'RCVMBRCO', 'SNDMBRCO', 'SNDBRHCO', 'SNDCLKNO', 'SNDTRDAT', 'SNDTRTIM', 'MSGFLGNO', 'ORMFN', 'OPRTYPNO', 'ROPRTPNO', 'SNDBNKNM', 'RCVBNKCO', 'RCVBNKNM', 'CUR', 'OCCAMT', 'CUSCHRG', 'PYRMBRCO', 'PYRACC', 'PYEMBRCO', 'PYEACC', 'ORTRCCO', 'ORTRCNO', 'DCFLG', 'CBFLG', 'CONFFLG', 'CANCFLG', 'STRINFO', 'BJEDTE', 'BSPSQN', 'NOTE1', 'NOTE2', 'NOTE3', 'NOTE4')
table_dict = {'TRCDAT': 'S', 'ORTRCCO': 'S', 'MSGFLGNO': 'S', 'SNDTRTIM': 'S', 'PYRACC': 'S', 'STRINFO': 'S', 'SNDMBRCO': 'S', 'MSGTYPCO': 'S', 'CONFFLG': 'S', 'RCVMBRCO': 'S', 'OCCAMT': 'F', 'CUR': 'S', 'BJEDTE': 'S', 'DCFLG': 'S', 'CBFLG': 'S', 'RCVBNKCO': 'S', 'SNDCLKNO': 'S', 'ROPRTPNO': 'S', 'ORMFN': 'S', 'PYRMBRCO': 'S', 'ORTRCNO': 'S', 'SNDBNKNM': 'S', 'TRCCO': 'S', 'RCVBNKNM': 'S', 'NCCWKDAT': 'S', 'PYEACC': 'S', 'PYEMBRCO': 'S', 'CUSCHRG': 'F', 'OPRTYPNO': 'S', 'NOTE1': 'S', 'NOTE3': 'S', 'NOTE2': 'S', 'NOTE4': 'S', 'CANCFLG': 'S', 'SNDBRHCO': 'S', 'SNDBNKCO': 'S', 'TRCNO': 'S', 'BSPSQN': 'S', 'SNDTRDAT': 'S'}
    
def chk(where_dict):
    for item in where_dict.keys():
        if item not in table_tuple:
            AfaLoggerFunc.tradeError( '表[rcc_tddzmx]中无列[' + item + ']' )
            return False
    return True
    
def count( wheresql ):

    sql = "SELECT count(*) FROM rcc_tddzmx WHERE " + wheresql
...
def insert( insert_dict ):
    ...
    sql = "INSERT INTO rcc_tddzmx (" + ','.join(table_tuple) + ") VALUES ( "
...
def update( update_dict,where_dict ):
    ...
    sql = "UPDATE rcc_tddzmx SET " + updatesql[0:-1] + " WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
...
def delete( where_dict ):
    ...
    sql = "DELETE FROM rcc_tddzmx WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
