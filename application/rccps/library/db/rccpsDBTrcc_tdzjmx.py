# -*- coding: gbk -*-
##################################################################
#   农信银系统 rcc_tdzjmx 数据库表操作函数
#
#   作    者：  关彬捷
#   程序文件:   rccpsDBTrcc_tdzjmx.py
#   修改时间:   Tue Dec 16 19:39:24 2008
##################################################################
import AfaDBFunc,AfaLoggerFunc
from types import *

table_tuple = ('NCCWKDAT', 'SCNBBH', 'SCFEDT', 'SCRBSQ', 'SCEYDT', 'SCTLSQ', 'SCRVSB', 'SCCATR', 'SCWLBZ', 'SCTRAM', 'SCFLAG', 'SCSTCD', 'SCBYZ1', 'SCBYZ2', 'SCBYZ3', 'NOTE1', 'NOTE2', 'NOTE3', 'NOTE4')
table_dict = {'SCNBBH': 'S', 'NOTE2': 'S', 'SCRBSQ': 'S', 'NCCWKDAT': 'S', 'SCCATR': 'S', 'SCTLSQ': 'S', 'NOTE3': 'S', 'SCFEDT': 'S', 'NOTE4': 'S', 'SCBYZ1': 'F', 'NOTE1': 'S', 'SCBYZ3': 'S', 'SCBYZ2': 'S', 'SCTRAM': 'F', 'SCEYDT': 'S', 'SCWLBZ': 'S', 'SCFLAG': 'S', 'SCRVSB': 'S', 'SCSTCD': 'S'}
    
def chk(where_dict):
    for item in where_dict.keys():
        if item not in table_tuple:
            AfaLoggerFunc.tradeError( '表[rcc_tdzjmx]中无列[' + item + ']' )
            return False
    return True
    
def count( wheresql ):

    sql = "SELECT count(*) FROM rcc_tdzjmx WHERE " + wheresql
...
def selectm( start_no, sel_size, wheresql, ordersql):
    ...
    sql = "SELECT NCCWKDAT,SCNBBH,SCFEDT,SCRBSQ,SCEYDT,SCTLSQ,SCRVSB,SCCATR,SCWLBZ,SCTRAM,SCFLAG,SCSTCD,SCBYZ1,SCBYZ2,SCBYZ3,NOTE1,NOTE2,NOTE3,NOTE4 FROM (SELECT row_number() over(" + ordersql + ") as rowid,NCCWKDAT,SCNBBH,SCFEDT,SCRBSQ,SCEYDT,SCTLSQ,SCRVSB,SCCATR,SCWLBZ,SCTRAM,SCFLAG,SCSTCD,SCBYZ1,SCBYZ2,SCBYZ3,NOTE1,NOTE2,NOTE3,NOTE4 from rcc_tdzjmx WHERE " + wheresql + " ) as tab1 where tab1.rowid >= " + str(start_no)
    AfaLoggerFunc.tradeInfo(sql)
...
def insert( insert_dict ):
    ...
    sql = "INSERT INTO rcc_tdzjmx (" + ','.join(table_tuple) + ") VALUES ( "
...
def update( update_dict,where_dict ):
    ...
    sql = "UPDATE rcc_tdzjmx SET " + updatesql[0:-1] + " WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
...
def delete( where_dict ):
    ...
    sql = "DELETE FROM rcc_tdzjmx WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
