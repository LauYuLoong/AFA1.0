# -*- coding: gbk -*-
##################################################################
#   农信银系统 rcc_subbra 数据库表操作函数
#
#   作    者：  关彬捷
#   程序文件:   rccpsDBTrcc_subbra.py
#   修改时间:   Tue Dec 16 19:39:23 2008
##################################################################
import AfaDBFunc,AfaLoggerFunc
from types import *

table_tuple = ('BESBNO', 'BESBNM', 'BESBTP', 'BTOPSB', 'BEACSB', 'BANKBIN', 'STRINFO', 'SUBFLG', 'NOTE1', 'NOTE2', 'NOTE3', 'NOTE4')
table_dict = {'BEACSB': 'S', 'BESBTP': 'S', 'NOTE1': 'S', 'NOTE3': 'S', 'NOTE2': 'S', 'BTOPSB': 'S', 'BANKBIN': 'S', 'STRINFO': 'S', 'SUBFLG': 'S', 'NOTE4': 'S', 'BESBNM': 'S', 'BESBNO': 'S'}
    
def chk(where_dict):
    for item in where_dict.keys():
        if item not in table_tuple:
            AfaLoggerFunc.tradeError( '表[rcc_subbra]中无列[' + item + ']' )
            return False
    return True
    
def count( wheresql ):

    sql = "SELECT count(*) FROM rcc_subbra WHERE " + wheresql
...
def selectm( start_no, sel_size, wheresql, ordersql):
    ...
    sql = "SELECT BESBNO,BESBNM,BESBTP,BTOPSB,BEACSB,BANKBIN,STRINFO,SUBFLG,NOTE1,NOTE2,NOTE3,NOTE4 FROM (SELECT row_number() over(" + ordersql + ") as rowid,BESBNO,BESBNM,BESBTP,BTOPSB,BEACSB,BANKBIN,STRINFO,SUBFLG,NOTE1,NOTE2,NOTE3,NOTE4 from rcc_subbra WHERE " + wheresql + " ) as tab1 where tab1.rowid >= " + str(start_no)
    AfaLoggerFunc.tradeInfo(sql)
...
def insert( insert_dict ):
    ...
    sql = "INSERT INTO rcc_subbra (" + ','.join(table_tuple) + ") VALUES ( "
...
def update( update_dict,where_dict ):
    ...
    sql = "UPDATE rcc_subbra SET " + updatesql[0:-1] + " WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
...
def delete( where_dict ):
    ...
    sql = "DELETE FROM rcc_subbra WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
