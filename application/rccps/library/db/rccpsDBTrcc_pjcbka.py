# -*- coding: gbk -*-
##################################################################
#   农信银系统 rcc_pjcbka 数据库表操作函数
#
#   作    者：  关彬捷
#   程序文件:   rccpsDBTrcc_pjcbka.py
#   修改时间:   Tue Dec 16 19:39:22 2008
##################################################################
import AfaDBFunc,AfaLoggerFunc
from types import *

table_tuple = ('BJEDTE', 'BSPSQN', 'BRSFLG', 'BESBNO', 'BETELR', 'BEAUUS', 'NCCWKDAT', 'TRCCO', 'TRCDAT', 'TRCNO', 'SNDMBRCO', 'RCVMBRCO', 'SNDBNKCO', 'SNDBNKNM', 'RCVBNKCO', 'RCVBNKNM', 'BOJEDT', 'BOSPSQ', 'ORTRCCO', 'BILDAT', 'BILNO', 'BILPNAM', 'BILENDDT', 'BILAMT', 'PYENAM', 'HONBNKNM', 'CONT', 'ISDEAL', 'PRCCO', 'STRINFO', 'NOTE1', 'NOTE2', 'NOTE3', 'NOTE4')
table_dict = {'BEAUUS': 'S', 'CONT': 'S', 'TRCDAT': 'S', 'ORTRCCO': 'S', 'PRCCO': 'S', 'SNDMBRCO': 'S', 'RCVMBRCO': 'S', 'PYENAM': 'S', 'BJEDTE': 'S', 'BILPNAM': 'S', 'BSPSQN': 'S', 'RCVBNKCO': 'S', 'BOSPSQ': 'S', 'SNDBNKNM': 'S', 'BILENDDT': 'S', 'TRCCO': 'S', 'RCVBNKNM': 'S', 'NCCWKDAT': 'S', 'BRSFLG': 'S', 'BILNO': 'S', 'BILDAT': 'S', 'BILAMT': 'F', 'STRINFO': 'S', 'HONBNKNM': 'S', 'ISDEAL': 'S', 'BOJEDT': 'S', 'NOTE1': 'S', 'NOTE3': 'S', 'NOTE2': 'S', 'NOTE4': 'S', 'SNDBNKCO': 'S', 'TRCNO': 'S', 'BESBNO': 'S', 'BETELR': 'S'}
    
def chk(where_dict):
    for item in where_dict.keys():
        if item not in table_tuple:
            AfaLoggerFunc.tradeError( '表[rcc_pjcbka]中无列[' + item + ']' )
            return False
    return True
    
def count( wheresql ):

    sql = "SELECT count(*) FROM rcc_pjcbka WHERE " + wheresql
...
def insert( insert_dict ):
    ...
    sql = "INSERT INTO rcc_pjcbka (" + ','.join(table_tuple) + ") VALUES ( "
...
def update( update_dict,where_dict ):
    ...
    sql = "UPDATE rcc_pjcbka SET " + updatesql[0:-1] + " WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
...
def delete( where_dict ):
    ...
    sql = "DELETE FROM rcc_pjcbka WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
