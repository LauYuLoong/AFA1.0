# -*- coding: gbk -*-
##################################################################
#   农信银系统 rcc_chgtbl 数据库表操作函数
#
#   作    者：  关彬捷
#   程序文件:   rccpsDBTrcc_chgtbl.py
#   修改时间:   Tue Dec 16 19:39:19 2008
##################################################################
import AfaDBFunc,AfaLoggerFunc
from types import *

table_tuple = ('BESBNO', 'BPSBNO', 'CHGTYP', 'TRCCO', 'CHGNAM', 'CHGDATA', 'EFCTDAT', 'INVDATE', 'UPDTIME')
table_dict = {'EFCTDAT': 'S', 'UPDTIME': 'S', 'TRCCO': 'S', 'CHGTYP': 'S', 'INVDATE': 'S', 'BPSBNO': 'S', 'BESBNO': 'S', 'CHGDATA': 'S', 'CHGNAM': 'S'}
    
def chk(where_dict):
    for item in where_dict.keys():
        if item not in table_tuple:
            AfaLoggerFunc.tradeError( '表[rcc_chgtbl]中无列[' + item + ']' )
            return False
    return True
    
def count( wheresql ):

    sql = "SELECT count(*) FROM rcc_chgtbl WHERE " + wheresql
...
def selectm( start_no, sel_size, wheresql, ordersql):
    ...
    sql = "SELECT BESBNO,BPSBNO,CHGTYP,TRCCO,CHGNAM,CHGDATA,EFCTDAT,INVDATE,UPDTIME FROM (SELECT row_number() over(" + ordersql + ") as rowid,BESBNO,BPSBNO,CHGTYP,TRCCO,CHGNAM,CHGDATA,EFCTDAT,INVDATE,UPDTIME from rcc_chgtbl WHERE " + wheresql + " ) as tab1 where tab1.rowid >= " + str(start_no)
    AfaLoggerFunc.tradeInfo(sql)
...
def insert( insert_dict ):
    ...
    sql = "INSERT INTO rcc_chgtbl (" + ','.join(table_tuple) + ") VALUES ( "
...
def update( update_dict,where_dict ):
    ...
    sql = "UPDATE rcc_chgtbl SET " + updatesql[0:-1] + " WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
...
def delete( where_dict ):
    ...
    sql = "DELETE FROM rcc_chgtbl WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
