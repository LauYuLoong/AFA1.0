# -*- coding: gbk -*-
##################################################################
#   农信银系统 rcc_existp 数据库表操作函数
#
#   作    者：  关彬捷
#   程序文件:   rccpsDBTrcc_existp.py
#   修改时间:   Tue Dec 16 19:39:20 2008
##################################################################
import AfaDBFunc,AfaLoggerFunc
from types import *

table_tuple = ('BJEDTE', 'BSPSQN', 'BRSFLG', 'BESBNO', 'BETELR', 'BEAUUS', 'NCCWKDAT', 'TRCCO', 'TRCDAT', 'TRCNO', 'SNDMBRCO', 'RCVMBRCO', 'SNDBNKCO', 'SNDBNKNM', 'RCVBNKCO', 'RCVBNKNM', 'BOJEDT', 'BOSPSQ', 'ORTRCCO', 'CUR', 'OCCAMT', 'CONT', 'PRCCO', 'STRINFO', 'NOTE1', 'NOTE2', 'NOTE3', 'NOTE4')
table_dict = {'BEAUUS': 'S', 'CONT': 'S', 'TRCDAT': 'S', 'ORTRCCO': 'S', 'PRCCO': 'S', 'SNDMBRCO': 'S', 'RCVMBRCO': 'S', 'OCCAMT': 'F', 'CUR': 'S', 'BJEDTE': 'S', 'BSPSQN': 'S', 'RCVBNKCO': 'S', 'BOSPSQ': 'S', 'SNDBNKNM': 'S', 'TRCCO': 'S', 'RCVBNKNM': 'S', 'NCCWKDAT': 'S', 'BRSFLG': 'S', 'STRINFO': 'S', 'BOJEDT': 'S', 'NOTE1': 'S', 'NOTE3': 'S', 'NOTE2': 'S', 'NOTE4': 'S', 'SNDBNKCO': 'S', 'TRCNO': 'S', 'BESBNO': 'S', 'BETELR': 'S'}
    
def chk(where_dict):
    for item in where_dict.keys():
        if item not in table_tuple:
            AfaLoggerFunc.tradeError( '表[rcc_existp]中无列[' + item + ']' )
            return False
    return True
    
def count( wheresql ):

    sql = "SELECT count(*) FROM rcc_existp WHERE " + wheresql
...
def selectu( where_dict ):
    ...
    sql = "SELECT BJEDTE,BSPSQN,BRSFLG,BESBNO,BETELR,BEAUUS,NCCWKDAT,TRCCO,TRCDAT,TRCNO,SNDMBRCO,RCVMBRCO,SNDBNKCO,SNDBNKNM,RCVBNKCO,RCVBNKNM,BOJEDT,BOSPSQ,ORTRCCO,CUR,OCCAMT,CONT,PRCCO,STRINFO,NOTE1,NOTE2,NOTE3,NOTE4 FROM rcc_existp WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
    records = AfaDBFunc.SelectSql( sql )
    if (records == None):
        return None
...
def selectm( start_no, sel_size, wheresql, ordersql):
    ...
    sql = "SELECT BJEDTE,BSPSQN,BRSFLG,BESBNO,BETELR,BEAUUS,NCCWKDAT,TRCCO,TRCDAT,TRCNO,SNDMBRCO,RCVMBRCO,SNDBNKCO,SNDBNKNM,RCVBNKCO,RCVBNKNM,BOJEDT,BOSPSQ,ORTRCCO,CUR,OCCAMT,CONT,PRCCO,STRINFO,NOTE1,NOTE2,NOTE3,NOTE4 FROM (SELECT row_number() over(" + ordersql + ") as rowid,BJEDTE,BSPSQN,BRSFLG,BESBNO,BETELR,BEAUUS,NCCWKDAT,TRCCO,TRCDAT,TRCNO,SNDMBRCO,RCVMBRCO,SNDBNKCO,SNDBNKNM,RCVBNKCO,RCVBNKNM,BOJEDT,BOSPSQ,ORTRCCO,CUR,OCCAMT,CONT,PRCCO,STRINFO,NOTE1,NOTE2,NOTE3,NOTE4 from rcc_existp WHERE " + wheresql + " ) as tab1 where tab1.rowid >= " + str(start_no)
    AfaLoggerFunc.tradeInfo(sql)
...
def insert( insert_dict ):
    ...
    sql = "INSERT INTO rcc_existp (" + ','.join(table_tuple) + ") VALUES ( "
...
def update( update_dict,where_dict ):
    ...
    sql = "UPDATE rcc_existp SET " + updatesql[0:-1] + " WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
...
def delete( where_dict ):
    ...
    sql = "DELETE FROM rcc_existp WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
