# -*- coding: gbk -*-
##################################################################
#   农信银系统 rcc_pamtbl 数据库表操作函数
#
#   作    者：  关彬捷
#   程序文件:   rccpsDBTrcc_pamtbl.py
#   修改时间:   Tue Dec 16 19:39:22 2008
##################################################################
import AfaDBFunc,AfaLoggerFunc
from types import *

table_tuple = ('BPATPE', 'BPARAD', 'BPACMT', 'BPADAT', 'BPAINF', 'BEFTDT', 'BINVDT', 'NOTE1', 'NOTE2', 'NOTE3', 'NOTE4')
table_dict = {'BPADAT': 'S', 'NOTE2': 'S', 'NOTE1': 'S', 'BPAINF': 'S', 'BPACMT': 'S', 'NOTE4': 'S', 'BPARAD': 'S', 'BPATPE': 'S', 'BINVDT': 'S', 'NOTE3': 'S', 'BEFTDT': 'S'}
    
def chk(where_dict):
    for item in where_dict.keys():
        if item not in table_tuple:
            AfaLoggerFunc.tradeError( '表[rcc_pamtbl]中无列[' + item + ']' )
            return False
    return True
    
def count( wheresql ):

    sql = "SELECT count(*) FROM rcc_pamtbl WHERE " + wheresql
...
def selectm( start_no, sel_size, wheresql, ordersql):
    ...
    sql = "SELECT BPATPE,BPARAD,BPACMT,BPADAT,BPAINF,BEFTDT,BINVDT,NOTE1,NOTE2,NOTE3,NOTE4 FROM (SELECT row_number() over(" + ordersql + ") as rowid,BPATPE,BPARAD,BPACMT,BPADAT,BPAINF,BEFTDT,BINVDT,NOTE1,NOTE2,NOTE3,NOTE4 from rcc_pamtbl WHERE " + wheresql + " ) as tab1 where tab1.rowid >= " + str(start_no)
    AfaLoggerFunc.tradeInfo(sql)
...
def insert( insert_dict ):
    ...
    sql = "INSERT INTO rcc_pamtbl (" + ','.join(table_tuple) + ") VALUES ( "
...
def update( update_dict,where_dict ):
    ...
    sql = "UPDATE rcc_pamtbl SET " + updatesql[0:-1] + " WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
...
def delete( where_dict ):
    ...
    sql = "DELETE FROM rcc_pamtbl WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
