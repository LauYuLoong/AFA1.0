# -*- coding: gbk -*-
##################################################################
#   农信银系统 rcc_cshalm 数据库表操作函数
#
#   作    者：  关彬捷
#   程序文件:   rccpsDBTrcc_cshalm.py
#   修改时间:   Tue Dec 16 19:39:20 2008
##################################################################
import AfaDBFunc,AfaLoggerFunc
from types import *

table_tuple = ('BJEDTE', 'BSPSQN', 'NCCWKDAT', 'TRCCO', 'TRCDAT', 'TRCNO', 'SNDBNKCO', 'SNDBNKNM', 'RCVBNKCO', 'RCVBNKNM', 'CUR', 'POSITION', 'POSALAMT', 'NOTE1', 'NOTE2', 'NOTE3', 'NOTE4')
table_dict = {'BJEDTE': 'S', 'POSALAMT': 'F', 'TRCCO': 'S', 'RCVBNKNM': 'S', 'NCCWKDAT': 'S', 'TRCDAT': 'S', 'NOTE1': 'S', 'BSPSQN': 'S', 'RCVBNKCO': 'S', 'NOTE2': 'S', 'NOTE4': 'S', 'SNDBNKCO': 'S', 'TRCNO': 'S', 'POSITION': 'F', 'SNDBNKNM': 'S', 'NOTE3': 'S', 'CUR': 'S'}
    
def chk(where_dict):
    for item in where_dict.keys():
        if item not in table_tuple:
            AfaLoggerFunc.tradeError( '表[rcc_cshalm]中无列[' + item + ']' )
            return False
    return True
    
def count( wheresql ):

    sql = "SELECT count(*) FROM rcc_cshalm WHERE " + wheresql
...
def selectu( where_dict ):
    ...
    sql = "SELECT BJEDTE,BSPSQN,NCCWKDAT,TRCCO,TRCDAT,TRCNO,SNDBNKCO,SNDBNKNM,RCVBNKCO,RCVBNKNM,CUR,POSITION,POSALAMT,NOTE1,NOTE2,NOTE3,NOTE4 FROM rcc_cshalm WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
    records = AfaDBFunc.SelectSql( sql )
    if (records == None):
        return None
...
def selectm( start_no, sel_size, wheresql, ordersql):
    ...
    sql = "SELECT BJEDTE,BSPSQN,NCCWKDAT,TRCCO,TRCDAT,TRCNO,SNDBNKCO,SNDBNKNM,RCVBNKCO,RCVBNKNM,CUR,POSITION,POSALAMT,NOTE1,NOTE2,NOTE3,NOTE4 FROM (SELECT row_number() over(" + ordersql + ") as rowid,BJEDTE,BSPSQN,NCCWKDAT,TRCCO,TRCDAT,TRCNO,SNDBNKCO,SNDBNKNM,RCVBNKCO,RCVBNKNM,CUR,POSITION,POSALAMT,NOTE1,NOTE2,NOTE3,NOTE4 from rcc_cshalm WHERE " + wheresql + " ) as tab1 where tab1.rowid >= " + str(start_no)
    AfaLoggerFunc.tradeInfo(sql)
...
def insert( insert_dict ):
    ...
    sql = "INSERT INTO rcc_cshalm (" + ','.join(table_tuple) + ") VALUES ( "
...
def update( update_dict,where_dict ):
    ...
    sql = "UPDATE rcc_cshalm SET " + updatesql[0:-1] + " WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
...
def delete( where_dict ):
    ...
    sql = "DELETE FROM rcc_cshalm WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
