# -*- coding: gbk -*-
##################################################################
#   农信银系统 rcc_hpcbka 数据库表操作函数
#
#   作    者：  关彬捷
#   程序文件:   rccpsDBTrcc_hpcbka.py
#   修改时间:   Tue Dec 16 19:39:21 2008
##################################################################
import AfaDBFunc,AfaLoggerFunc
from types import *

table_tuple = ('BJEDTE', 'BSPSQN', 'BRSFLG', 'BESBNO', 'BETELR', 'BEAUUS', 'NCCWKDAT', 'TRCCO', 'TRCDAT', 'TRCNO', 'SNDMBRCO', 'RCVMBRCO', 'SNDBNKCO', 'SNDBNKNM', 'RCVBNKCO', 'RCVBNKNM', 'BOJEDT', 'BOSPSQ', 'ORTRCCO', 'BILVER', 'BILNO', 'BILDAT', 'PAYWAY', 'CUR', 'BILAMT', 'PYRACC', 'PYRNAM', 'PYEACC', 'PYENAM', 'CONT', 'ISDEAL', 'PRCCO', 'STRINFO', 'NOTE1', 'NOTE2', 'NOTE3', 'NOTE4')
table_dict = {'BEAUUS': 'S', 'CONT': 'S', 'TRCDAT': 'S', 'ORTRCCO': 'S', 'PYRACC': 'S', 'PRCCO': 'S', 'SNDMBRCO': 'S', 'PYRNAM': 'S', 'RCVMBRCO': 'S', 'PYENAM': 'S', 'CUR': 'S', 'BJEDTE': 'S', 'BSPSQN': 'S', 'RCVBNKCO': 'S', 'BOSPSQ': 'S', 'SNDBNKNM': 'S', 'TRCCO': 'S', 'RCVBNKNM': 'S', 'NCCWKDAT': 'S', 'BRSFLG': 'S', 'BILNO': 'S', 'BILDAT': 'S', 'PAYWAY': 'S', 'BILAMT': 'F', 'STRINFO': 'S', 'BILVER': 'S', 'ISDEAL': 'S', 'BOJEDT': 'S', 'NOTE1': 'S', 'NOTE3': 'S', 'NOTE2': 'S', 'NOTE4': 'S', 'SNDBNKCO': 'S', 'PYEACC': 'S', 'TRCNO': 'S', 'BESBNO': 'S', 'BETELR': 'S'}
    
def chk(where_dict):
    for item in where_dict.keys():
        if item not in table_tuple:
            AfaLoggerFunc.tradeError( '表[rcc_hpcbka]中无列[' + item + ']' )
            return False
    return True
    
def count( wheresql ):

    sql = "SELECT count(*) FROM rcc_hpcbka WHERE " + wheresql
...
def selectu( where_dict ):
    ...
    sql = "SELECT BJEDTE,BSPSQN,BRSFLG,BESBNO,BETELR,BEAUUS,NCCWKDAT,TRCCO,TRCDAT,TRCNO,SNDMBRCO,RCVMBRCO,SNDBNKCO,SNDBNKNM,RCVBNKCO,RCVBNKNM,BOJEDT,BOSPSQ,ORTRCCO,BILVER,BILNO,BILDAT,PAYWAY,CUR,BILAMT,PYRACC,PYRNAM,PYEACC,PYENAM,CONT,ISDEAL,PRCCO,STRINFO,NOTE1,NOTE2,NOTE3,NOTE4 FROM rcc_hpcbka WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
    records = AfaDBFunc.SelectSql( sql )
    if (records == None):
        return None
...
def selectm( start_no, sel_size, wheresql, ordersql):
    ...
    sql = "SELECT BJEDTE,BSPSQN,BRSFLG,BESBNO,BETELR,BEAUUS,NCCWKDAT,TRCCO,TRCDAT,TRCNO,SNDMBRCO,RCVMBRCO,SNDBNKCO,SNDBNKNM,RCVBNKCO,RCVBNKNM,BOJEDT,BOSPSQ,ORTRCCO,BILVER,BILNO,BILDAT,PAYWAY,CUR,BILAMT,PYRACC,PYRNAM,PYEACC,PYENAM,CONT,ISDEAL,PRCCO,STRINFO,NOTE1,NOTE2,NOTE3,NOTE4 FROM (SELECT row_number() over(" + ordersql + ") as rowid,BJEDTE,BSPSQN,BRSFLG,BESBNO,BETELR,BEAUUS,NCCWKDAT,TRCCO,TRCDAT,TRCNO,SNDMBRCO,RCVMBRCO,SNDBNKCO,SNDBNKNM,RCVBNKCO,RCVBNKNM,BOJEDT,BOSPSQ,ORTRCCO,BILVER,BILNO,BILDAT,PAYWAY,CUR,BILAMT,PYRACC,PYRNAM,PYEACC,PYENAM,CONT,ISDEAL,PRCCO,STRINFO,NOTE1,NOTE2,NOTE3,NOTE4 from rcc_hpcbka WHERE " + wheresql + " ) as tab1 where tab1.rowid >= " + str(start_no)
    AfaLoggerFunc.tradeInfo(sql)
...
def insert( insert_dict ):
    ...
    sql = "INSERT INTO rcc_hpcbka (" + ','.join(table_tuple) + ") VALUES ( "
...
def update( update_dict,where_dict ):
    ...
    sql = "UPDATE rcc_hpcbka SET " + updatesql[0:-1] + " WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
...
def delete( where_dict ):
    ...
    sql = "DELETE FROM rcc_hpcbka WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
