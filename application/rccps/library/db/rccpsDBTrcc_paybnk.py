# -*- coding: gbk -*-
##################################################################
#   农信银系统 rcc_paybnk 数据库表操作函数
#
#   作    者：  关彬捷
#   程序文件:   rccpsDBTrcc_paybnk.py
#   修改时间:   Tue Dec 16 19:39:22 2008
##################################################################
import AfaDBFunc,AfaLoggerFunc
from types import *

table_tuple = ('BANKBIN', 'BANKSTATUS', 'BANKATTR', 'STLBANKBIN', 'BANKNAM', 'BANKADDR', 'BANKPC', 'BANKTEL', 'EFCTDAT', 'INVDAT', 'ALTTYPE', 'PRIVILEGE', 'NEWOFLG', 'STRINFO', 'NOTE1', 'NOTE2', 'NOTE3', 'NOTE4')
table_dict = {'EFCTDAT': 'S', 'STLBANKBIN': 'S', 'NOTE2': 'S', 'INVDAT': 'S', 'BANKADDR': 'S', 'NOTE1': 'S', 'BANKNAM': 'S', 'BANKSTATUS': 'S', 'BANKATTR': 'S', 'NOTE4': 'S', 'BANKBIN': 'S', 'STRINFO': 'S', 'ALTTYPE': 'S', 'NEWOFLG': 'S', 'PRIVILEGE': 'S', 'NOTE3': 'S', 'BANKTEL': 'S', 'BANKPC': 'S'}
    
def chk(where_dict):
    for item in where_dict.keys():
        if item not in table_tuple:
            AfaLoggerFunc.tradeError( '表[rcc_paybnk]中无列[' + item + ']' )
            return False
    return True
    
def count( wheresql ):

    sql = "SELECT count(*) FROM rcc_paybnk WHERE " + wheresql
...
def insert( insert_dict ):
    ...
    sql = "INSERT INTO rcc_paybnk (" + ','.join(table_tuple) + ") VALUES ( "
...
def update( update_dict,where_dict ):
    ...
    sql = "UPDATE rcc_paybnk SET " + updatesql[0:-1] + " WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
...
def delete( where_dict ):
    ...
    sql = "DELETE FROM rcc_paybnk WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
