# -*- coding: gbk -*-
##################################################################
#   农信银系统 rcc_cadbnk 数据库表操作函数
#
#   作    者：  关彬捷
#   程序文件:   rccpsDBTrcc_cadbnk.py
#   修改时间:   Tue Dec 16 19:39:19 2008
##################################################################
import AfaDBFunc,AfaLoggerFunc
from types import *

table_tuple = ('CARDBIN', 'BANKBIN', 'NOTE1', 'NOTE2', 'NOTE3', 'NOTE4')
table_dict = {'NOTE1': 'S', 'NOTE3': 'S', 'NOTE2': 'S', 'NOTE4': 'S', 'BANKBIN': 'S', 'CARDBIN': 'S'}
    
def chk(where_dict):
    for item in where_dict.keys():
        if item not in table_tuple:
            AfaLoggerFunc.tradeError( '表[rcc_cadbnk]中无列[' + item + ']' )
            return False
    return True
    
def count( wheresql ):

    sql = "SELECT count(*) FROM rcc_cadbnk WHERE " + wheresql
...
def selectu( where_dict ):
    ...
    sql = "SELECT CARDBIN,BANKBIN,NOTE1,NOTE2,NOTE3,NOTE4 FROM rcc_cadbnk WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
    records = AfaDBFunc.SelectSql( sql )
    if (records == None):
        return None
...
def selectm( start_no, sel_size, wheresql, ordersql):
    ...
    sql = "SELECT CARDBIN,BANKBIN,NOTE1,NOTE2,NOTE3,NOTE4 FROM (SELECT row_number() over(" + ordersql + ") as rowid,CARDBIN,BANKBIN,NOTE1,NOTE2,NOTE3,NOTE4 from rcc_cadbnk WHERE " + wheresql + " ) as tab1 where tab1.rowid >= " + str(start_no)
    AfaLoggerFunc.tradeInfo(sql)
...
def insert( insert_dict ):
    ...
    sql = "INSERT INTO rcc_cadbnk (" + ','.join(table_tuple) + ") VALUES ( "
...
def update( update_dict,where_dict ):
    ...
    sql = "UPDATE rcc_cadbnk SET " + updatesql[0:-1] + " WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
...
def delete( where_dict ):
    ...
    sql = "DELETE FROM rcc_cadbnk WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
