# -*- coding: gbk -*-
##################################################################
#   农信银系统 rcc_hddzhz 数据库表操作函数
#
#   作    者：  关彬捷
#   程序文件:   rccpsDBTrcc_hddzhz.py
#   修改时间:   Tue Dec 16 19:39:20 2008
##################################################################
import AfaDBFunc,AfaLoggerFunc
from types import *

table_tuple = ('NCCWKDAT', 'TRCCO', 'BRSFLG', 'TRCNAM', 'TRCRSNM', 'TCNT', 'CTAMT', 'DTAMT', 'OFSTAMT', 'NOTE1', 'NOTE2', 'NOTE3', 'NOTE4')
table_dict = {'TRCCO': 'S', 'NCCWKDAT': 'S', 'BRSFLG': 'S', 'TRCRSNM': 'S', 'TCNT': 'N', 'NOTE3': 'S', 'NOTE2': 'S', 'NOTE4': 'S', 'TRCNAM': 'S', 'DTAMT': 'F', 'OFSTAMT': 'F', 'CTAMT': 'F', 'NOTE1': 'S'}
    
def chk(where_dict):
    for item in where_dict.keys():
        if item not in table_tuple:
            AfaLoggerFunc.tradeError( '表[rcc_hddzhz]中无列[' + item + ']' )
            return False
    return True
    
def count( wheresql ):

    sql = "SELECT count(*) FROM rcc_hddzhz WHERE " + wheresql
...
def selectu( where_dict ):
    ...
    sql = "SELECT NCCWKDAT,TRCCO,BRSFLG,TRCNAM,TRCRSNM,TCNT,CTAMT,DTAMT,OFSTAMT,NOTE1,NOTE2,NOTE3,NOTE4 FROM rcc_hddzhz WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
    records = AfaDBFunc.SelectSql( sql )
    if (records == None):
        return None
...
def selectm( start_no, sel_size, wheresql, ordersql):
    ...
    sql = "SELECT NCCWKDAT,TRCCO,BRSFLG,TRCNAM,TRCRSNM,TCNT,CTAMT,DTAMT,OFSTAMT,NOTE1,NOTE2,NOTE3,NOTE4 FROM (SELECT row_number() over(" + ordersql + ") as rowid,NCCWKDAT,TRCCO,BRSFLG,TRCNAM,TRCRSNM,TCNT,CTAMT,DTAMT,OFSTAMT,NOTE1,NOTE2,NOTE3,NOTE4 from rcc_hddzhz WHERE " + wheresql + " ) as tab1 where tab1.rowid >= " + str(start_no)
    AfaLoggerFunc.tradeInfo(sql)
...
def insert( insert_dict ):
    ...
    sql = "INSERT INTO rcc_hddzhz (" + ','.join(table_tuple) + ") VALUES ( "
...
def update( update_dict,where_dict ):
    ...
    sql = "UPDATE rcc_hddzhz SET " + updatesql[0:-1] + " WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
...
def delete( where_dict ):
    ...
    sql = "DELETE FROM rcc_hddzhz WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
