# -*- coding: gbk -*-
##################################################################
#   农信银系统 rcc_acckj 数据库表操作函数
#
#   作    者：  曾照泰
#   程序文件:   rccpsDBTrcc_acckj.py
#   修改时间:   20110522
##################################################################
import AfaDBFunc,AfaLoggerFunc
from types import *

table_tuple = ('TRCDAT','BSPSQN','MSGFLGNO','ORMFN','TRCCO','BRSFLG','BESBNO','BEACSB','BETELR','BEAUUS','BEAUPS','TERMID','OPTYPE','NCCWKDAT','TRCNO','SNDMBRCO','RCVMBRCO','SNDBNKCO','SNDBNKNM','RCVBNKCO','RCVBNKNM','ORTRCDAT','ORTRCCO','ORTRCNO','ORSNDSUBBNK','ORSNDBNK','ORRCVSUBBNK','ORRCVBNK','ORPYRACC','ORPYRNAM','ORPYEACC','ORPYENAM','CUR','OCCAMT','CHRG','ERRCONBAL','BALANCE','UNCONRST','CONSTS','PRCCO','STRINFO','NOTE1','NOTE2','NOTE3','NOTE4','NOTE5')
table_dict = {'BEAUUS': 'S','ORMFN' :'S','TRCDAT': 'S','OPTYPE':'S','NCCWKDAT':'S', 'MSGFLGNO': 'S','TRCNO':'S', 'TRCCO': 'S', 'STRINFO': 'S', 'SNDMBRCO': 'S', 'RCVMBRCO': 'S','ORTRCNO':'S','ORPYRNAM':'S','ORPYEACC':'S','ORTRCDAT':'S','ORTRCCO':'S','ORSNDSUBBNK':'S','ORSNDBNK':'S','ORRCVSUBBNK':'S','ORRCVBNK':'S','ORPYRACC':'S', 'ORPYENAM':'S','CUR': 'S','OCCAMT':'F','CHRG':'F', 'ERRCONBAL':'F','BALANCE':'F','UNCONRST':'S','CONSTS':'S','BJEDTE': 'S', 'BNKBKNO': 'S', 'BSPSQN': 'S', 'RCVBNKCO': 'S', 'OPRATTNO': 'S', 'CERTTYPE': 'S', 'TERMID': 'S', 'SNDBNKNM': 'S', 'BEACSB': 'S', 'RCVBNKNM': 'S', 'BRSFLG': 'S', 'CERTNO': 'S',  'PRCCO': 'S', 'NOTE1': 'S', 'OPRNO': 'S', 'NOTE3': 'S', 'NOTE2': 'S', 'NOTE4': 'S', 'NOTE5':'S','BEAUPS': 'S', 'SNDBNKCO': 'S', 'TRCNO': 'S', 'BESBNO': 'S', 'BETELR': 'S','NOTE1':'S'}
    
def chk(where_dict):
    for item in where_dict.keys():
        if item not in table_tuple:
            AfaLoggerFunc.tradeError( '表[rcc_acckj]中无列[' + item + ']' )
            return False
    return True
    
def count( wheresql ):

    sql = "SELECT count(*) FROM rcc_acckj WHERE " + wheresql
...
def selectm( start_no, sel_size, wheresql, ordersql):
    ...
    sql = "SELECT TRCDAT,BSPSQN,MSGFLGNO,ORMFN,TRCCO,BRSFLG,BESBNO,BEACSB,BETELR,BEAUUS,BEAUPS,TERMID,OPTYPE,NCCWKDAT,TRCNO,SNDMBRCO,RCVMBRCO,SNDBNKCO,SNDBNKNM,RCVBNKCO,RCVBNKNM,ORTRCDAT,ORTRCCO,ORTRCNO,ORSNDSUBBNK,ORSNDBNK,ORRCVSUBBNK,ORRCVBNK,ORPYRACC,ORPYRNAM,ORPYEACC,ORPYENAM,CUR,OCCAMT,CHRG,ERRCONBAL,BALANCE,UNCONRST,CONSTS,PRCCO,STRINFO,NOTE1,NOTE2,NOTE3,NOTE4,NOTE5 from rcc_acckj WHERE " + wheresql + " ) as tab1 where tab1.rowid >= " + str(start_no)
    AfaLoggerFunc.tradeInfo(sql)
...
def insert( insert_dict ):
    ...
    sql = "INSERT INTO rcc_acckj (" + ','.join(table_tuple) + ") VALUES ( "
...
def update( update_dict,where_dict ):
    ...
    sql = "UPDATE rcc_acckj SET " + updatesql[0:-1] + " WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
...
def delete( where_dict ):
    ...
    sql = "DELETE FROM rcc_acckj WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
