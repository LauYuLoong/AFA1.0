# -*- coding: gbk -*-
##################################################################
#   农信银系统 rcc_atrchk 数据库表操作函数
#
#   作    者：  关彬捷
#   程序文件:   rccpsDBTrcc_atrchk.py
#   修改时间:   Tue Dec 16 19:39:19 2008
##################################################################
import AfaDBFunc,AfaLoggerFunc
from types import *

table_tuple = ('SNDBNKCO', 'TRCDAT', 'TRCNO', 'ROPRTPNO', 'TRCCO', 'RCVBNKCO', 'CUR', 'RDTCNT', 'RDTAMT', 'RCTCNT', 'RCTAMT', 'SDTCNT', 'SDTAMT', 'SCTCNT', 'SCTAMT', 'NOTE1', 'NOTE2', 'NOTE3', 'NOTE4')
table_dict = {'TRCCO': 'S', 'CUR': 'S', 'TRCDAT': 'S', 'NOTE1': 'S', 'SCTCNT': 'N', 'RCVBNKCO': 'S', 'NOTE2': 'S', 'SDTCNT': 'N', 'SDTAMT': 'F', 'RDTCNT': 'N', 'SNDBNKCO': 'S', 'NOTE4': 'S', 'TRCNO': 'S', 'ROPRTPNO': 'S', 'NOTE3': 'S', 'RDTAMT': 'F', 'RCTCNT': 'N', 'SCTAMT': 'F', 'RCTAMT': 'F'}
...
def count( wheresql ):
    ...
    sql = "SELECT count(*) FROM rcc_atrchk WHERE " + wheresql
...
def selectm( start_no, sel_size, wheresql, ordersql):
    ...
    sql = "SELECT SNDBNKCO,TRCDAT,TRCNO,ROPRTPNO,TRCCO,RCVBNKCO,CUR,RDTCNT,RDTAMT,RCTCNT,RCTAMT,SDTCNT,SDTAMT,SCTCNT,SCTAMT,NOTE1,NOTE2,NOTE3,NOTE4 FROM (SELECT row_number() over(" + ordersql + ") as rowid,SNDBNKCO,TRCDAT,TRCNO,ROPRTPNO,TRCCO,RCVBNKCO,CUR,RDTCNT,RDTAMT,RCTCNT,RCTAMT,SDTCNT,SDTAMT,SCTCNT,SCTAMT,NOTE1,NOTE2,NOTE3,NOTE4 from rcc_atrchk WHERE " + wheresql + " ) as tab1 where tab1.rowid >= " + str(start_no)
    AfaLoggerFunc.tradeInfo(sql)
...
def insert( insert_dict ):
    ...
    sql = "INSERT INTO rcc_atrchk (" + ','.join(table_tuple) + ") VALUES ( "
...
def update( update_dict,where_dict ):
    ...
    sql = "UPDATE rcc_atrchk SET " + updatesql[0:-1] + " WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
...
def delete( where_dict ):
    ...
    sql = "DELETE FROM rcc_atrchk WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
