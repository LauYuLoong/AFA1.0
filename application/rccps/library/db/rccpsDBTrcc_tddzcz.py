# -*- coding: gbk -*-
##################################################################
#   农信银系统 rcc_tddzcz 数据库表操作函数
#
#   作    者：  关彬捷
#   程序文件:   rccpsDBTrcc_tddzcz.py
#   修改时间:   Tue Dec 16 19:39:23 2008
##################################################################
import AfaDBFunc,AfaLoggerFunc
from types import *

table_tuple = ('NCCWKDAT', 'SNDBNKCO', 'TRCDAT', 'TRCNO', 'RCVBNKCO', 'SNDMBRCO', 'RCVMBRCO', 'TRCCO', 'DCFLG', 'PYRACC', 'PYEACC', 'CUR', 'OCCAMT', 'LOCOCCAMT', 'CUSCHRG', 'LOCCUSCHRG', 'ORTRCNO', 'BJEDTE', 'BSPSQN', 'EACTYP', 'EACINF', 'LOCEACTYP', 'LOCEACINF', 'ISDEAL', 'NOTE1', 'NOTE2', 'NOTE3', 'NOTE4')
table_dict = {'TRCDAT': 'S', 'EACINF': 'S', 'PYRACC': 'S', 'SNDMBRCO': 'S', 'RCVMBRCO': 'S', 'OCCAMT': 'F', 'LOCEACINF': 'S', 'CUR': 'S', 'BJEDTE': 'S', 'DCFLG': 'S', 'BSPSQN': 'S', 'EACTYP': 'S', 'ORTRCNO': 'S', 'TRCCO': 'S', 'NCCWKDAT': 'S', 'PYEACC': 'S', 'LOCOCCAMT': 'F', 'LOCEACTYP': 'S', 'ISDEAL': 'S', 'CUSCHRG': 'F', 'LOCCUSCHRG': 'F', 'NOTE1': 'S', 'NOTE3': 'S', 'NOTE2': 'S', 'NOTE4': 'S', 'SNDBNKCO': 'S', 'TRCNO': 'S', 'RCVBNKCO': 'S'}
    
def chk(where_dict):
    for item in where_dict.keys():
        if item not in table_tuple:
            AfaLoggerFunc.tradeError( '表[rcc_tddzcz]中无列[' + item + ']' )
            return False
    return True
    
def count( wheresql ):

    sql = "SELECT count(*) FROM rcc_tddzcz WHERE " + wheresql
...
def insert( insert_dict ):
    ...
    sql = "INSERT INTO rcc_tddzcz (" + ','.join(table_tuple) + ") VALUES ( "
...
def update( update_dict,where_dict ):
    ...
    sql = "UPDATE rcc_tddzcz SET " + updatesql[0:-1] + " WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
...
def delete( where_dict ):
    ...
    sql = "DELETE FROM rcc_tddzcz WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
