# -*- coding: gbk -*-
##################################################################
#   农信银系统 rcc_hddzmx 数据库表操作函数
#
#   作    者：  关彬捷
#   程序文件:   rccpsDBTrcc_hddzmx.py
#   修改时间:   Tue Dec 16 19:39:20 2008
##################################################################
import AfaDBFunc,AfaLoggerFunc
from types import *

table_tuple = ('SNDBNKCO', 'TRCDAT', 'TRCNO', 'NCCWKDAT', 'MSGTYPCO', 'RCVMBRCO', 'SNDMBRCO', 'TRCCO', 'SNDBRHCO', 'SNDCLKNO', 'SNDTRDAT', 'SNDTRTIM', 'MSGFLGNO', 'ORMFN', 'OPRTYPNO', 'ROPRTPNO', 'OPRSTNO', 'SNDBNKNM', 'RCVBNKCO', 'RCVBNKNM', 'CUR', 'OCCAMT', 'PYRACC', 'PYRNAM', 'PYRADDR', 'PYEACC', 'PYENAM', 'PYEADDR', 'OPRATTNO', 'SEAL', 'ORTRCCO', 'ORSNDBNK', 'ORRCVBNK', 'ORTRCDAT', 'ORTRCNO', 'REMARK', 'BILDAT', 'BILNO', 'BILTYP', 'CPSAMT', 'RFUAMT', 'STRINFO', 'USE', 'BJEDTE', 'BSPSQN', 'BCSTAT', 'BDWFLG', 'EACTYP', 'NOTE1', 'NOTE2', 'NOTE3', 'NOTE4')
table_dict = {'OPRSTNO': 'S', 'NOTE2': 'S', 'TRCDAT': 'S', 'ORTRCCO': 'S', 'MSGFLGNO': 'S', 'SNDTRTIM': 'S', 'PYRACC': 'S', 'STRINFO': 'S', 'SNDMBRCO': 'S', 'MSGTYPCO': 'S', 'ORSNDBNK': 'S', 'SEAL': 'S', 'RCVMBRCO': 'S', 'OCCAMT': 'F', 'PYENAM': 'S', 'CUR': 'S', 'BJEDTE': 'S', 'REMARK': 'S', 'BILTYP': 'S', 'BSPSQN': 'S', 'RCVBNKCO': 'S', 'OPRATTNO': 'S', 'SNDCLKNO': 'S', 'ROPRTPNO': 'S', 'ORMFN': 'S', 'ORTRCNO': 'S', 'SNDBNKNM': 'S', 'USE': 'S', 'TRCCO': 'S', 'RCVBNKNM': 'S', 'PYEADDR': 'S', 'NCCWKDAT': 'S', 'ORRCVBNK': 'S', 'BILNO': 'S', 'BILDAT': 'S', 'BCSTAT': 'S', 'PYEACC': 'S', 'CPSAMT': 'F', 'PYRADDR': 'S', 'OPRTYPNO': 'S', 'PYRNAM': 'S', 'NOTE1': 'S', 'NOTE3': 'S', 'RFUAMT': 'F', 'NOTE4': 'S', 'SNDBRHCO': 'S', 'SNDBNKCO': 'S', 'ORTRCDAT': 'S', 'TRCNO': 'S', 'BDWFLG': 'S', 'SNDTRDAT': 'S', 'EACTYP': 'S'}
    
def chk(where_dict):
    for item in where_dict.keys():
        if item not in table_tuple:
            AfaLoggerFunc.tradeError( '表[rcc_hddzmx]中无列[' + item + ']' )
            return False
    return True
    
def count( wheresql ):

    sql = "SELECT count(*) FROM rcc_hddzmx WHERE " + wheresql
...
def selectu( where_dict ):
    ...
    sql = "SELECT SNDBNKCO,TRCDAT,TRCNO,NCCWKDAT,MSGTYPCO,RCVMBRCO,SNDMBRCO,TRCCO,SNDBRHCO,SNDCLKNO,SNDTRDAT,SNDTRTIM,MSGFLGNO,ORMFN,OPRTYPNO,ROPRTPNO,OPRSTNO,SNDBNKNM,RCVBNKCO,RCVBNKNM,CUR,OCCAMT,PYRACC,PYRNAM,PYRADDR,PYEACC,PYENAM,PYEADDR,OPRATTNO,SEAL,ORTRCCO,ORSNDBNK,ORRCVBNK,ORTRCDAT,ORTRCNO,REMARK,BILDAT,BILNO,BILTYP,CPSAMT,RFUAMT,STRINFO,USE,BJEDTE,BSPSQN,BCSTAT,BDWFLG,EACTYP,NOTE1,NOTE2,NOTE3,NOTE4 FROM rcc_hddzmx WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
    records = AfaDBFunc.SelectSql( sql )
    if (records == None):
        return None
...
def selectm( start_no, sel_size, wheresql, ordersql):
    ...
    sql = "SELECT SNDBNKCO,TRCDAT,TRCNO,NCCWKDAT,MSGTYPCO,RCVMBRCO,SNDMBRCO,TRCCO,SNDBRHCO,SNDCLKNO,SNDTRDAT,SNDTRTIM,MSGFLGNO,ORMFN,OPRTYPNO,ROPRTPNO,OPRSTNO,SNDBNKNM,RCVBNKCO,RCVBNKNM,CUR,OCCAMT,PYRACC,PYRNAM,PYRADDR,PYEACC,PYENAM,PYEADDR,OPRATTNO,SEAL,ORTRCCO,ORSNDBNK,ORRCVBNK,ORTRCDAT,ORTRCNO,REMARK,BILDAT,BILNO,BILTYP,CPSAMT,RFUAMT,STRINFO,USE,BJEDTE,BSPSQN,BCSTAT,BDWFLG,EACTYP,NOTE1,NOTE2,NOTE3,NOTE4 FROM (SELECT row_number() over(" + ordersql + ") as rowid,SNDBNKCO,TRCDAT,TRCNO,NCCWKDAT,MSGTYPCO,RCVMBRCO,SNDMBRCO,TRCCO,SNDBRHCO,SNDCLKNO,SNDTRDAT,SNDTRTIM,MSGFLGNO,ORMFN,OPRTYPNO,ROPRTPNO,OPRSTNO,SNDBNKNM,RCVBNKCO,RCVBNKNM,CUR,OCCAMT,PYRACC,PYRNAM,PYRADDR,PYEACC,PYENAM,PYEADDR,OPRATTNO,SEAL,ORTRCCO,ORSNDBNK,ORRCVBNK,ORTRCDAT,ORTRCNO,REMARK,BILDAT,BILNO,BILTYP,CPSAMT,RFUAMT,STRINFO,USE,BJEDTE,BSPSQN,BCSTAT,BDWFLG,EACTYP,NOTE1,NOTE2,NOTE3,NOTE4 from rcc_hddzmx WHERE " + wheresql + " ) as tab1 where tab1.rowid >= " + str(start_no)
    AfaLoggerFunc.tradeInfo(sql)
...
def insert( insert_dict ):
    ...
    sql = "INSERT INTO rcc_hddzmx (" + ','.join(table_tuple) + ") VALUES ( "
...
def update( update_dict,where_dict ):
    ...
    sql = "UPDATE rcc_hddzmx SET " + updatesql[0:-1] + " WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
...
def delete( where_dict ):
    ...
    sql = "DELETE FROM rcc_hddzmx WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
