# -*- coding: gbk -*-
##################################################################
#   农信银系统 rcc_tdzjcz 数据库表操作函数
#
#   作    者：  关彬捷
#   程序文件:   rccpsDBTrcc_tdzjcz.py
#   修改时间:   Tue Dec 16 19:39:23 2008
##################################################################
import AfaDBFunc,AfaLoggerFunc
from types import *

table_tuple = ('NCCWKDAT', 'SCFEDT', 'SCRBSQ', 'RCFEDT', 'RCRBSQ', 'SCTRAM', 'RCTRAM', 'SCTRDT', 'SCTLSQ', 'ERRTYP', 'ERRINF', 'ISDEAL', 'NOTE1', 'NOTE2', 'NOTE3', 'NOTE4')
table_dict = {'SCTRDT': 'S', 'RCRBSQ': 'S', 'SCRBSQ': 'S', 'NCCWKDAT': 'S', 'SCTLSQ': 'S', 'NOTE3': 'S', 'SCFEDT': 'S', 'NOTE4': 'S', 'ERRTYP': 'S', 'NOTE1': 'S', 'RCTRAM': 'F', 'SCTRAM': 'F', 'NOTE2': 'S', 'RCFEDT': 'S', 'ERRINF': 'S', 'ISDEAL': 'S'}
    
def chk(where_dict):
    for item in where_dict.keys():
        if item not in table_tuple:
            AfaLoggerFunc.tradeError( '表[rcc_tdzjcz]中无列[' + item + ']' )
            return False
    return True
    
def count( wheresql ):

    sql = "SELECT count(*) FROM rcc_tdzjcz WHERE " + wheresql
...
def selectu( where_dict ):
    ...
    sql = "SELECT NCCWKDAT,SCFEDT,SCRBSQ,RCFEDT,RCRBSQ,SCTRAM,RCTRAM,SCTRDT,SCTLSQ,ERRTYP,ERRINF,ISDEAL,NOTE1,NOTE2,NOTE3,NOTE4 FROM rcc_tdzjcz WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
    records = AfaDBFunc.SelectSql( sql )
    if (records == None):
        return None
...
def selectm( start_no, sel_size, wheresql, ordersql):
    ...
    sql = "SELECT NCCWKDAT,SCFEDT,SCRBSQ,RCFEDT,RCRBSQ,SCTRAM,RCTRAM,SCTRDT,SCTLSQ,ERRTYP,ERRINF,ISDEAL,NOTE1,NOTE2,NOTE3,NOTE4 FROM (SELECT row_number() over(" + ordersql + ") as rowid,NCCWKDAT,SCFEDT,SCRBSQ,RCFEDT,RCRBSQ,SCTRAM,RCTRAM,SCTRDT,SCTLSQ,ERRTYP,ERRINF,ISDEAL,NOTE1,NOTE2,NOTE3,NOTE4 from rcc_tdzjcz WHERE " + wheresql + " ) as tab1 where tab1.rowid >= " + str(start_no)
    AfaLoggerFunc.tradeInfo(sql)
...
def insert( insert_dict ):
    ...
    sql = "INSERT INTO rcc_tdzjcz (" + ','.join(table_tuple) + ") VALUES ( "
...
def update( update_dict,where_dict ):
    ...
    sql = "UPDATE rcc_tdzjcz SET " + updatesql[0:-1] + " WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
