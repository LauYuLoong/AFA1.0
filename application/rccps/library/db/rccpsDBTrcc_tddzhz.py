# -*- coding: gbk -*-
##################################################################
#   农信银系统 rcc_tddzhz 数据库表操作函数
#
#   作    者：  关彬捷
#   程序文件:   rccpsDBTrcc_tddzhz.py
#   修改时间:   Tue Dec 16 19:39:23 2008
##################################################################
import AfaDBFunc,AfaLoggerFunc
from types import *

table_tuple = ('NCCWKDAT', 'TRCCO', 'BRSFLG', 'TRCNAM', 'TRCRSNM', 'TCNT', 'CTAMT', 'DTAMT', 'CHRCTAMT', 'CHRDTAMT', 'OFSTAMT', 'NOTE1', 'NOTE2', 'NOTE3', 'NOTE4')
table_dict = {'TRCCO': 'S', 'NCCWKDAT': 'S', 'BRSFLG': 'S', 'TRCRSNM': 'S', 'TCNT': 'N', 'NOTE3': 'S', 'NOTE2': 'S', 'CHRCTAMT': 'F', 'CHRDTAMT': 'F', 'TRCNAM': 'S', 'DTAMT': 'F', 'NOTE4': 'S', 'OFSTAMT': 'F', 'CTAMT': 'F', 'NOTE1': 'S'}
    
def chk(where_dict):
    for item in where_dict.keys():
        if item not in table_tuple:
            AfaLoggerFunc.tradeError( '表[rcc_tddzhz]中无列[' + item + ']' )
            return False
    return True
    
def count( wheresql ):

    sql = "SELECT count(*) FROM rcc_tddzhz WHERE " + wheresql
...
def selectu( where_dict ):
    ...
    sql = "SELECT NCCWKDAT,TRCCO,BRSFLG,TRCNAM,TRCRSNM,TCNT,CTAMT,DTAMT,CHRCTAMT,CHRDTAMT,OFSTAMT,NOTE1,NOTE2,NOTE3,NOTE4 FROM rcc_tddzhz WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
    records = AfaDBFunc.SelectSql( sql )
    if (records == None):
        return None
...
def selectm( start_no, sel_size, wheresql, ordersql):
    ...
    sql = "SELECT NCCWKDAT,TRCCO,BRSFLG,TRCNAM,TRCRSNM,TCNT,CTAMT,DTAMT,CHRCTAMT,CHRDTAMT,OFSTAMT,NOTE1,NOTE2,NOTE3,NOTE4 FROM (SELECT row_number() over(" + ordersql + ") as rowid,NCCWKDAT,TRCCO,BRSFLG,TRCNAM,TRCRSNM,TCNT,CTAMT,DTAMT,CHRCTAMT,CHRDTAMT,OFSTAMT,NOTE1,NOTE2,NOTE3,NOTE4 from rcc_tddzhz WHERE " + wheresql + " ) as tab1 where tab1.rowid >= " + str(start_no)
    AfaLoggerFunc.tradeInfo(sql)
...
def insert( insert_dict ):
    ...
    sql = "INSERT INTO rcc_tddzhz (" + ','.join(table_tuple) + ") VALUES ( "
...
def update( update_dict,where_dict ):
    ...
    sql = "UPDATE rcc_tddzhz SET " + updatesql[0:-1] + " WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
...
def delete( where_dict ):
    ...
    sql = "DELETE FROM rcc_tddzhz WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
