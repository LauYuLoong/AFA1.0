# -*- coding: gbk -*-
##################################################################
#   农信银系统 rcc_qsqhqd 数据库表操作函数
#
#   作    者：  关彬捷
#   程序文件:   rccpsDBTrcc_qsqhqd.py
#   修改时间:   Tue Dec 16 19:39:22 2008
##################################################################
import AfaDBFunc,AfaLoggerFunc
from types import *

table_tuple = ('STRDAT', 'ENDDAT', 'BESBNO', 'BESBNM', 'TRCCHRGCNT', 'TRCCHRGAMT', 'BILCHRGCNT', 'BILCHRGAMT', 'UCSWCHRGCNT', 'UCSWCHRGAMT', 'TCNT', 'TAMT', 'ISDEAL', 'FEDT', 'RBSQ', 'TRDT', 'TLSQ', 'MGID')
table_dict = {'STRDAT': 'S', 'TRCCHRGCNT': 'N', 'BILCHRGAMT': 'F', 'ISDEAL': 'S', 'TCNT': 'F', 'UCSWCHRGAMT': 'F', 'UCSWCHRGCNT': 'N', 'TRCCHRGAMT': 'F', 'FEDT': 'S', 'TLSQ': 'S', 'BESBNM': 'S', 'BESBNO': 'S', 'RBSQ': 'S', 'ENDDAT': 'S', 'TRDT': 'S', 'MGID': 'S', 'BILCHRGCNT': 'N', 'TAMT': 'F'}
    
def chk(where_dict):
    for item in where_dict.keys():
        if item not in table_tuple:
            AfaLoggerFunc.tradeError( '表[rcc_qsqhqd]中无列[' + item + ']' )
            return False
    return True
    
def count( wheresql ):

    sql = "SELECT count(*) FROM rcc_qsqhqd WHERE " + wheresql
...
def insert( insert_dict ):
    ...
    sql = "INSERT INTO rcc_qsqhqd (" + ','.join(table_tuple) + ") VALUES ( "
...
def update( update_dict,where_dict ):
    ...
    sql = "UPDATE rcc_qsqhqd SET " + updatesql[0:-1] + " WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
...
def delete( where_dict ):
    ...
    sql = "DELETE FROM rcc_qsqhqd WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
