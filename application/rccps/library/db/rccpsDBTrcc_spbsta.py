# -*- coding: gbk -*-
##################################################################
#   农信银系统 rcc_spbsta 数据库表操作函数
#
#   作    者：  关彬捷
#   程序文件:   rccpsDBTrcc_spbsta.py
#   修改时间:   Tue Dec 16 19:39:23 2008
##################################################################
import AfaDBFunc,AfaLoggerFunc
from types import *

table_tuple = ('BJEDTE', 'BSPSQN', 'BCURSQ', 'BCSTAT', 'BDWFLG', 'NOTE1', 'NOTE2', 'NOTE3', 'NOTE4')
table_dict = {'BJEDTE': 'S', 'BCURSQ': 'N', 'NOTE1': 'S', 'BSPSQN': 'S', 'NOTE3': 'S', 'NOTE2': 'S', 'NOTE4': 'S', 'BCSTAT': 'S', 'BDWFLG': 'S'}
    
def chk(where_dict):
    for item in where_dict.keys():
        if item not in table_tuple:
            AfaLoggerFunc.tradeError( '表[rcc_spbsta]中无列[' + item + ']' )
            return False
    return True
    
def count( wheresql ):

    sql = "SELECT count(*) FROM rcc_spbsta WHERE " + wheresql
...
def selectm( start_no, sel_size, wheresql, ordersql):
    ...
    sql = "SELECT BJEDTE,BSPSQN,BCURSQ,BCSTAT,BDWFLG,NOTE1,NOTE2,NOTE3,NOTE4 FROM (SELECT row_number() over(" + ordersql + ") as rowid,BJEDTE,BSPSQN,BCURSQ,BCSTAT,BDWFLG,NOTE1,NOTE2,NOTE3,NOTE4 from rcc_spbsta WHERE " + wheresql + " ) as tab1 where tab1.rowid >= " + str(start_no)
    AfaLoggerFunc.tradeInfo(sql)
...
def insert( insert_dict ):
    ...
    sql = "INSERT INTO rcc_spbsta (" + ','.join(table_tuple) + ") VALUES ( "
...
def update( update_dict,where_dict ):
    ...
    sql = "UPDATE rcc_spbsta SET " + updatesql[0:-1] + " WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
...
def delete( where_dict ):
    ...
    sql = "DELETE FROM rcc_spbsta WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
