# -*- coding: gbk -*-
##################################################################
#   农信银系统 rcc_hpdzhz 数据库表操作函数
#
#   作    者：  关彬捷
#   程序文件:   rccpsDBTrcc_hpdzhz.py
#   修改时间:   Tue Dec 16 19:39:21 2008
##################################################################
import AfaDBFunc,AfaLoggerFunc
from types import *

table_tuple = ('NCCWKDAT', 'TRCCO', 'BRSFLG', 'TRCNAM', 'TRCRSNM', 'TCNT', 'CTAMT', 'DTAMT', 'OFSTAMT', 'CLAMT', 'DLAMT', 'OFSLAMT', 'NOTE1', 'NOTE2', 'NOTE3', 'NOTE4')
table_dict = {'TRCCO': 'S', 'DLAMT': 'F', 'CLAMT': 'F', 'NCCWKDAT': 'S', 'BRSFLG': 'S', 'TRCRSNM': 'S', 'TCNT': 'N', 'NOTE3': 'S', 'NOTE2': 'S', 'NOTE4': 'S', 'TRCNAM': 'S', 'DTAMT': 'F', 'OFSLAMT': 'F', 'OFSTAMT': 'F', 'CTAMT': 'F', 'NOTE1': 'S'}
    
def chk(where_dict):
    for item in where_dict.keys():
        if item not in table_tuple:
            AfaLoggerFunc.tradeError( '表[rcc_hpdzhz]中无列[' + item + ']' )
            return False
    return True
    
def count( wheresql ):

    sql = "SELECT count(*) FROM rcc_hpdzhz WHERE " + wheresql
...
def insert( insert_dict ):
    ...
    sql = "INSERT INTO rcc_hpdzhz (" + ','.join(table_tuple) + ") VALUES ( "
...
def update( update_dict,where_dict ):
    ...
    sql = "UPDATE rcc_hpdzhz SET " + updatesql[0:-1] + " WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
...
def delete( where_dict ):
    ...
    sql = "DELETE FROM rcc_hpdzhz WHERE " + wheresql[0:-5]
    AfaLoggerFunc.tradeInfo(sql)
