# -*- coding: gbk -*-
##################################################################
#   农信银系统 stat_dict 字典到 wtr_dict 字典映射函数
#
#   作    者：  关彬捷
#   程序文件:   rccpsMap0000Dstat_dict2Dwtr_dict.py
#   修改时间:   Tue Dec  2 17:14:44 2008
##################################################################
...
def map(from_dict,to_dict):
    ...
    if from_dict.has_key('MGID'):
        ...
        AfaLoggerFunc.tradeDebug('wtr_dict[MGID] = ' + str(to_dict['MGID']))
    else:
        AfaLoggerFunc.tradeDebug("stat_dict['MGID']不存在")

    if from_dict.has_key('PRCCO'):
        to_dict['PRCCO'] = from_dict['PRCCO']
        AfaLoggerFunc.tradeDebug('wtr_dict[PRCCO] = ' + str(to_dict['PRCCO']))
    else:
