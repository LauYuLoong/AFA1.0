...
def MainTH_Proc( ):
    ...
    try:
        ...
        sql = "SELECT * FROM ABDT_BATCHINFO WHERE STATUS IN ('22','30','31')"
        ...
        while ( i  < len(records) ):
            ...
            AbdtManager.WrtLog('提交:APPNO=[' + TradeContext.APPNO + '],BATCHNO=[' + TradeContext.BATCHNO + ']')
            ...
            if ( TradeContext.STATUS == "22" ):
                AbdtManager.WrtLog('已提交[22]:[' + TradeContext.BATCHNO + ']')
                AbdtManager.SC_Proc(TradeContext.BATCHNO)
                continue


            if ( TradeContext.STATUS == "30" ):
                AbdtManager.WrtLog('待提回[30]:[' + TradeContext.BATCHNO + ']')
                AbdtManager.CX_Proc(TradeContext.BATCHNO)
                continue

    
            if ( TradeContext.STATUS == "31" ):
                AbdtManager.WrtLog('正在提回[31]:[' + TradeContext.BATCHNO + ']')
        ...
        return AbdtManager.ExitThisFlow( '9000', '提回批次信息(提回)异常')
