...
def MatchData(sysId,unitNo,trxDate):
    ...
    try:
        ...
        while ( len(linebuf) > 0 ):
            ...
            #先查询数据库中记录是否存在
            sqlstr = "SELECT BRNO,TELLERNO,REVTRANF,AMOUNT,BANKSTATUS,CORPSTATUS FROM AFA_MAINTRANSDTL WHERE"
            ...
            #修改与数据库进行匹配
            updSql = "UPDATE AFA_MAINTRANSDTL SET CHKFLAG='" + ChkFlag + "' WHERE"
            updSql = updSql + " SYSID='"        + sysId              + "'"
