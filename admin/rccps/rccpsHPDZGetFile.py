# -*- coding: gbk -*-
################################################################################
#   农信银系统：系统调度类.汇票对账文件下载
#===============================================================================
#   交易文件:   rccpsHPDZGetFile.py
#   公司名称：  北京赞同科技有限公司
#   作    者：  关彬捷
#   修改时间:   2008-06-24
################################################################################
...
if __name__ == '__main__':
    ...
    try:
        AfaLoggerFunc.tradeInfo("***农信银系统: 系统调度类.汇票对账文件下载[rccpsHPDZGetFile]进入***")
        ...
        #================判断前置机汇票对账文件是否传送完毕=========================
        AfaLoggerFunc.tradeInfo(">>>开始判断前置机汇票对账文件是否传送完毕")
        ...
        if file_line[:8] != NCCWKDAT:
            rccpsCronFunc.cronExit("S999","汇票对账文件尚未传送完毕")
        
        AfaLoggerFunc.tradeInfo(">>>汇票对账文件传送完毕")
        
        AfaLoggerFunc.tradeInfo(">>>结束判断前置机汇票对账文件是否传送完毕")
        
        #================下载汇票对账文件下载=======================================
        AfaLoggerFunc.tradeInfo(">>>开始下载汇票对账文件")
        
        file_path = "settlefile/HPHZCNY1340000008" + NCCWKDAT
        
        if not rccpsFtpFunc.getRccps(file_path):
            rccpsCronFunc.cronExit("S999","下载汇票对账汇总文件[" + file_path + "]异常")
        
        file_path = "settlefile/HPMXCNY1340000008" + NCCWKDAT
        
        if not rccpsFtpFunc.getRccps(file_path):
            rccpsCronFunc.cronExit("S999","下载汇票对账明细文件[" + file_path + "]异常")
        
        AfaLoggerFunc.tradeInfo(">>>结束下载汇票对账文件")
        
        #================关闭汇票对账文件下载系统调度,打开导入汇票对账文件系统调度==
        AfaLoggerFunc.tradeInfo(">>>开始关闭汇票对账文件下载系统调度,打开导入汇票对账文件系统调度")
        if not rccpsCronFunc.closeCron("00041"):
            rccpsCronFunc.cronExit("S999","关闭汇票对账文件下载系统调度异常")
            
        if not rccpsCronFunc.openCron("00044"):
            rccpsCronFunc.cronExit("S999","打开汇票对账明细文件导入系统调度异常")
        ...
        AfaLoggerFunc.tradeInfo(">>>结束关闭汇票对账文件下载系统调度,打开导入汇票对账文件系统调度")
        
        AfaLoggerFunc.tradeInfo("***农信银系统: 系统调度类.汇票对账文件下载[rccpsHPDZGetFile]退出***")
        ...
        if TradeContext.errorCode != '0000' :
            ...
            AfaLoggerFunc.tradeInfo('***[rccpsHPDZGetFile]交易中断***')
