# -*- coding: gbk -*-
################################################################################
#   农信银系统：系统调度类.农信银通存通兑文件生成及发送到主机
#===============================================================================
#   交易文件:   rccpsTDDZMXPutFile.py
#   公司名称：  北京赞同科技有限公司
#   作    者：  关彬捷
#   修改时间:   2008-12-10
################################################################################
...
if __name__ == '__main__':
    ...
    try:
        ...
        if tddzmx_list == None:
            rccpsCronFunc.cronExit("S999","查询通存通兑对账明细登记簿异常")
        ...
        if len(tddzmx_list) <= 0:
            ...
            for i in xrange(len(tddzmx_list)):
                ...
                file_line = file_line + str(tddzmx_list[i]['OCCAMT'])      + '|'     #交易金额
                file_line = file_line + str(tddzmx_list[i]['CUSCHRG'])      + '|'    #手续费金额
                file_line = file_line + "0".ljust(1,' ')                    + '|'    #记录状态
                file_line = file_line + "".ljust(62,' ')                  + '|'      #备用字段1
        ...
        AfaLoggerFunc.tradeInfo(">>>结束生成通存通兑主机对账文件")
        ...
        #================关闭通存通兑对账错账文件生成及发送到主机系统调度==========
        AfaLoggerFunc.tradeInfo(">>>开始关闭通存通兑对账错账文件生成及发送到主机系统调度")
        if not rccpsCronFunc.closeCron("00065"):
            rccpsCronFunc.cronExit("S999","关闭通存通兑对账错账文件生成及发送到主机调度异常")
        ...
        AfaLoggerFunc.tradeInfo(">>>结束关闭通存通兑对账错账文件生成及发送到主机系统调度")
        
        AfaLoggerFunc.tradeInfo("***农信银系统: 系统调度类.农信银通存通兑错账文件生成及发送到主机[rccpsHDDZMXPutFile]退出***")
