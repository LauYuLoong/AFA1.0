# -*- coding: gbk -*-
################################################################################
#   农信银系统：系统调度类.汇兑对账汇总文件导入
#===============================================================================
#   交易文件:   rccpsHDDZHZFileImport.py
#   公司名称：  北京赞同科技有限公司
#   作    者：  关彬捷
#   修改时间:   2008-06-25
################################################################################
...
if __name__ == '__main__':
    ...
    try:
        ...
        while file_line:
            ...
            hddzhz_insert_dict['NOTE1']    = NCCWKDAT          #对账日期
            
            if hddzhz_insert_dict['TRCRSNM'][:4] == "往账":
                hddzhz_insert_dict['BRSFLG'] = PL_BRSFLG_SND
            elif hddzhz_insert_dict['TRCRSNM'][:4] == "来账":
            ...
            if ret <= 0:
                AfaLoggerFunc.tradeInfo( AfaDBFunc.sqlErrMsg )
                rccpsCronFunc.cronExit("S999","导入汇兑对账汇总文件异常")
        
        AfaLoggerFunc.tradeInfo(">>>结束导入汇兑对账汇总文件")
        
        #====================关闭汇兑对账汇总文件导入系统调度,打开汇兑对账明细账勾兑====
        AfaLoggerFunc.tradeInfo(">>>开始关闭汇兑对账汇总文件导入系统调度,打开汇兑对账明细账勾兑")
        ...
        AfaLoggerFunc.tradeInfo("***农信银系统: 系统调度类.汇兑对账汇总文件导入[rccpsHDDZHZFileImport]退出***")
        ...
        if TradeContext.errorCode != '0000' :
            ...
            AfaLoggerFunc.tradeInfo('***[rccpsHDDZHZFileImport]交易中断***')
